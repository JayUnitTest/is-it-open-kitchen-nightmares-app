...
function App() {
  ...
  const handleSearch = (event) => {
    ...
    const result = data.find((restaurant) =>
  ...
  return (
    <div className="flex justify-center items-center h-screen">
      ...
      >
        <h1 className="text-2xl font-bold text-center text-[#B6EADA]">IS IT OPEN?</h1>
        ...
        {searchResult ? (
          <div className=" p-4 shadow mt-4">
            <h2 className="text-lg text-center font-semibold mb-2">{searchResult.name}</h2>
            <p className="mb-2 text-center ">
              Status: <span className="font-semibold">{searchResult.status}</span>
            </p>
            <p className="text-sm text-center ">{searchResult.description}</p>
          </div>
        ) : (
          <p className="mt-4">No matching restaurant found.</p>
        )}
      </div>
    </div>
  );
